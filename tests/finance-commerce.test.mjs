...
test("website catalog mirrors the 10 publicly verified Etsy listings", async () => {
  ...
  assert.deepEqual(
    Array.from(listings.slice(0, 5), (listing) => listing.featuredRank),
    [1, 2, 3, 4, 5]
  );
  ...
  for (const listing of listings) {
    ...
    assert.ok(listing.title.length > 30);
    assert.match(listing.priceDisplay, /^€ \d{1,2},\d{2}$/);
    assert.equal(listing.url, `https://www.etsy.com/listing/${listing.listingId}/`);
