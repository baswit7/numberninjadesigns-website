...
test("invalid dates and oversized windows are rejected before any request", () => {
  ...
  assert.throws(
    () => resolveDateRange({ startDate: "2026-02-31", endDate: "2026-03-01" }),
    /must use YYYY-MM-DD/
  );
  assert.throws(
...
test("invalid provider rows fail closed", async () => {
  await assert.rejects(
    () =>
      querySearchConsole(
        {
        ...
        },
        {
          fetchImpl: async () => ({
            ...
            status: 200,
            async json() {
              return { rows: [{ clicks: 1, impressions: 2, ctr: 0.5, position: 1 }] };
            }
          })
        }
      ),
