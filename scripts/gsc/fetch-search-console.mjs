...
export function resolveDateRange({ startDate, endDate, now = new Date() } = {}) {
  ...
  const spanDays =
    (Date.parse(`${resolvedEndDate}T00:00:00Z`) - Date.parse(`${resolvedStartDate}T00:00:00Z`)) /
      86400000 +
    1;
...
function normalizeRows(payload) {
  ...
  return rows.map((row, index) => {
    ...
    if (!datePattern.test(values.date || "") || row.keys.length !== 1) {
...
export async function querySearchConsole(
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    try {
      ...
      if (attempt === 3) throw new Error("Search Console query failed without an HTTP response");
