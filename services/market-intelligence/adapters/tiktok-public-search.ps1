...
foreach ($seed in $Seeds) {
    $url = $baseUrl + [uri]::EscapeDataString($seed)
    try {
        ...
        if ($content -match '(?i)captcha|verify you are human|login|sign in') {
            $riskNotes.Add("Search page blocked for query: $seed") | Out-Null
            continue
        }
        ...
        $riskNotes.Add("Search page request failed for query: $seed") | Out-Null
    }
}
...
$report = [ordered]@{
    sourceId = $sourceId
    status = $status
    sourceUrl = 'https://www.tiktok.com/search'
    checkedAt = $checkedAt
    query = ($Seeds -join ', ')
