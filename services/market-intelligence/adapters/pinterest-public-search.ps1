...
foreach ($seed in $Seeds) {
    $query = "$seed tech humor shirt"
    $url = 'https://www.pinterest.com/search/pins/?q=' + [uri]::EscapeDataString($query)
    try {
        $response = Invoke-WebRequest -Uri $url -Method GET -Headers $headers -TimeoutSec 18 -UseBasicParsing
        $content = [string]$response.Content
        if ($content -match '(?i)captcha|verify you are human|log in|sign up') {
            $riskNotes.Add("Pinterest public search blocked for query: $seed") | Out-Null
            continue
        }
        ...
        $riskNotes.Add("Pinterest public search request failed for query: $seed") | Out-Null
    }
}
...
$report = [ordered]@{
    sourceId = $sourceId
    status = $status
    sourceUrl = 'https://www.pinterest.com/search/pins/'
    checkedAt = $checkedAt
    query = ($Seeds -join ', ')
