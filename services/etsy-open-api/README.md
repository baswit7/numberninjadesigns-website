...
`ETSY_SHOP_ID` is the numeric owner shop ID. `ETSY_LISTING_CATALOG_PATH` is an
absolute server-side path to the publication catalog. The catalog and every
referenced asset must stay outside public web roots and inside the catalog's
directory tree. Product files are checked against Etsy's five-file and 20 MB
per-file limits before any provider write starts.
